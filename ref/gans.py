...
def create_model(inputs, targets):
    def create_generator(generator_inputs, generator_outputs_channels):
        ...
        layer_specs = [
            a.ngf * 2, # encoder_4: [batch, 32, 32, ngf * 4] => [batch, 16, 16, ngf * 8]
            a.ngf * 4, # encoder_5: [batch, 16, 16, ngf * 8] => [batch, 8, 8, ngf * 8]
        ...
        layer_specs = [
            ...
            (a.ngf * 2, 0.0),   # decoder_4: [batch, 16, 16, ngf * 8 * 2] => [batch, 32, 32, ngf * 4 * 2]
        ]
...
def save_images(fetches, image_dir, step=None):
    ...
    for i, in_path in enumerate(fetches["paths"]):
        ...
        for kind in ["inputs", "outputs", "targets"]:
            filename = str(name) + "-" + str(kind) + ".png"
            if step is not None:
